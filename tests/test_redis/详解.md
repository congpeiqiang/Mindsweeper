...
**含义**：向集合中添加一个或多个成员（集合会自动去重），若已存在则不添加，返回成功添加的数量
...
await redis.sadd("user:user_1001:tags", *tags)
```

![](D:\code_work_space\llm\huice\Issue-seven\Mindsweeper\tests\test_redis\images\sadd.png)
...
![](D:\code_work_space\llm\huice\Issue-seven\Mindsweeper\tests\test_redis\images\sadd-1.png)

## 6 .sismember 判断集合成员是否存在

```python
exstis_tag = await redis.sismember(tags_key, "Python")
```

**含义**：判断集合中的 某值 是否存在，返回1则存在，返回0则不存在
...
## 8 **.smembers** - 获取集合(set)所有成员

```python
await redis_client.smembers(key)
```

**含义**：获取集合中的所有成员，返回list数据类型
...
"""
返回
['Redis', '缓存', 'Python', '异步编程', '数据库', 'Docker', 'Kubernetes']
"""
```
...
```python
# 只返回字符串类型的键
for key in redis_client.scan_iter(match="session:*",type='string'):
    print(key.decode('utf-8'))
    
# 只返回哈希类型的键
for key in redis_client.scan_iter("session:*",type='hash'):
    print(key.decode('utf-8'))
    
# 只返回哈希类型的键
for key in redis_client.scan_iter("session:*",type='set'):
    print(key.decode('utf-8'))
```
...
**含义**：**查询keys对应的值的类型，查询类型包含: string、set、hash**
