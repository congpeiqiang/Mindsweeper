...
class MilvusDataInserter:
	...
	def __init__(self,
				 milvus_uri: str = "http://47.120.44.223:19530",
				 ollama_uri: str = "http://47.120.44.223:11434",
	...
	def _init_clients(self) -> None:
		...
		try:
			...
			# 初始化Embeddings客户端
			self.embeddings = OllamaEmbeddings(model="qwen3-embedding:0.6b", base_url=self.ollama_uri)
			logger.info("Embeddings客户端初始化成功")
	...
	def emb_text(self, text: str, max_retries: int = 3) -> List[float]:
		"""
		生成文本的向量嵌入
		...
		for attempt in range(max_retries):
			try:
				logger.info(f"正在为文本生成嵌入向量 (尝试 {attempt + 1}/{max_retries})")

				response = self.embeddings.embed_query(text)

				logger.info(f"成功生成 {len(response)} 维向量")
				return response
			...
			for doc_idx, doc_data in enumerate(tqdm(documents, desc="处理文档")):
				try:
					...
					for chunk_idx, chunk in enumerate(content_chunks):
						try:
							# 生成密集向量嵌入
							logger.info(f"正在为文档 {doc_data['docId']} 的块 {chunk_idx} 生成密集向量嵌入")
							...
							chunk_data = {
								...
								"content_chunk": chunk,  # 确保不超过长度限制
								"content_dense": dense_vector,
								"full_content": str(doc_data['content'])
							}
					...
					logger.info(f"文档 {doc_data['docId']} 已分割为 {len(content_chunks)} 个块")
...
if __name__ == "__main__":
	...
	inserter = MilvusDataInserter(
		milvus_uri="http://47.120.44.223:19530",
		ollama_uri = "http://47.120.44.223:11434",
