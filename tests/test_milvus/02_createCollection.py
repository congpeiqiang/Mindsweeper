    ...
    def create_schema(self) -> Optional[Any]:
        ...
        try:
            ...
            schema.add_field(
                ...
                # 启用 TEXT_MATCH
                # filter = "TEXT_MATCH(text, 'machine deep')" 搜索text 字段中包含machine 或deep 的文档
                # filter = "TEXT_MATCH(text, 'machine') and TEXT_MATCH(text, 'deep')" 搜索text 字段中同时包含machine 和deep 的文档
                # filter = "not TEXT_MATCH(text, 'deep') and TEXT_MATCH(text, 'machine') and TEXT_MATCH(text, 'learning')" 搜索text 字段中同时包含machine 和learning 但不包含deep 的文档
                # .search(filter=...)或者.query(filter=...)使用TEXT_MATCH
                enable_match=True,
...
if __name__ == "__main__":
    manager = MilvusCollectionManager(uri="http://47.120.44.223:19530", db_name="milvus_database")
    if manager.connect():
