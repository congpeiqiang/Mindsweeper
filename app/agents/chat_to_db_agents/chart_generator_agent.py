...
def _initialize_chart_client():
    """初始化图表生成客户端"""
    REQUEST_SERVER = "http://47.120.44.223:1122/sse"
    try:
...
def analyze_data_for_chart(data: Dict[str, Any], query_context: str) -> Dict[str, Any]:
    ...
    try:
        ...
        print(f"Tool: 分析数据特征，确定最适合的图表类型结果: {chart_recommendation}")
        return {
            "success": True,
            "data_analysis": {
...
# pragma: no cover  MS80OmFIVnBZMlhrdUp2bG43bmx2TG82ZW1kemRBPT06ZGQ5Y2FiZmQ=
...
# fmt: off  Mi80OmFIVnBZMlhrdUp2bG43bmx2TG82ZW1kemRBPT06ZGQ5Y2FiZmQ=
        ...
        if has_viz_intent or (has_numeric_data and len(columns) >= 2):
            print(f"Tool: 判断是否需要生成图表结果： True")
            return {
                "should_generate": True,
                "reason": "数据适合可视化且用户有可视化需求"
            }
        else:
            print(f"Tool: 判断是否需要生成图表结果： False")
            return {
                "should_generate": False,
...
# pylint: disable  My80OmFIVnBZMlhrdUp2bG43bmx2TG82ZW1kemRBPT06ZGQ5Y2FiZmQ=
