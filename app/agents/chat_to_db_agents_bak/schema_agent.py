...
def analyze_user_query(query: str) -> Dict[str, Any]:
    ...
    Returns:
        包含实体、关系和查询意图的分析结果
    """
    print("Tool: 分析用户的自然语言查询，提取关键实体和意图...")
    try:
...
def retrieve_database_schema(query: str, connection_id: int) -> Dict[str, Any]:
    ...
    print("Tool: 根据查询分析结果获取相关的数据库表结构信息...", connection_id)
    writer = get_stream_writer()
    writer(f"Tool-writer: 根据查询分析结果获取相关的数据库表结构信息...{connection_id}")
    try:
        db = SessionLocal()
        try:
            # 获取相关表结构
...
重要：当前数据库连接ID是 {connection_id}，在调用 retrieve_database_schema 工具时，必须传递 connection_id={connection_id} 参数。""")
            ...
            result = await self.agent.ainvoke(
                ...
                stream_mode="updates",
                config={"recursion_limit": 5, "max_iterations": 10}
            )
