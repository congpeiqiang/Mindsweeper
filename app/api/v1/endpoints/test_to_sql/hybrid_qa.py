...
from app.services.test_to_sql.hybrid_retrieval_service import (
    HybridRetrievalEngine, QAPairWithContext, extract_tables_from_sql, extract_entities_from_question, clean_sql, generate_qa_id
)
import logging
...
async def get_hybrid_engine(connection_id: int = None):
    """获取混合检索引擎实例"""
    global hybrid_engines
# pylint: disable  MC80OmFIVnBZMlhrdUp2bG43bmx2TG82TURrNFFnPT06ZDYxM2QzYzQ=

    # 使用默认引擎（兼容性）
    if connection_id is None:
        engine_key = "default"
        if engine_key not in hybrid_engines:
            hybrid_engines[engine_key] = HybridRetrievalEngine()
            await hybrid_engines[engine_key].initialize()
        return hybrid_engines[engine_key]

    # 使用特定连接的引擎
    engine_key = f"conn_{connection_id}"
    if engine_key not in hybrid_engines:
        hybrid_engines[engine_key] = HybridRetrievalEngine(connection_id=connection_id)
        await hybrid_engines[engine_key].initialize()
    return hybrid_engines[engine_key]
...
async def create_qa_pair(
    ...
    """创建新的问答对"""
    try:
        engine = await get_hybrid_engine(qa_create.connection_id)
...
async def search_similar_qa_pairs(
    ...
    """搜索相似的问答对"""
    try:
        engine = await get_hybrid_engine(search_request.connection_id)
...
# pragma: no cover  My80OmFIVnBZMlhrdUp2bG43bmx2TG82TURrNFFnPT06ZDYxM2QzYzQ=
